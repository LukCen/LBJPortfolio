...
<template>
  <section id="contact" class="flex flex-col gap-5 bg-medium p-4 rounded-xl shadow-primary">
    ...
    <form @submit.prevent="sendEmail" class="flex flex-col gap-5">
      ...
      <!-- submit -->
      <button type="submit" class="flex gap-5 px-10 py-2 w-fit bg-primary tracking-wider font-bold text-dark cursor-pointer hover:shadow-primary duration-125 rounded-lg">
        <Loader2 v-if="isSending" class="animate-spin" color="var(--color-dark)" />
        <span>Send</span>
      </button>
      <span ref="mailSuccessful" v-show="isSuccessful" :class="{ disabled: isSending === true }" class="font-semibold text-secondary" aria-live="polite">Your
        email was sent
